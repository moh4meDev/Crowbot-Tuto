...
module.exports = {
	...
	run: async (client, message, args, prefix, color) => {
		...
		if (client.config.owner.includes(message.author.id)) {
            ...
            embed.setDescription('**Une description complète et détaillée du Bot** <@1271862604414844961>')
            embed.setColor(color)
            .setThumbnail(message.author.avatarURL({ dynamic:true }));
            embed.setTimestamp()
            embed.setFooter(`${client.config.name}`)
            embed.addFields(
                { name: '👑 Owner / Developer :', value: '<@1260291217585868910>', inline: true },
                ...
                { name: '📞 Support :', value: `[Clique ICI Pour Rejoindre Le Support](https://guns.lol/faveck)`, inline: true },
